  ...
  static List<ExerciseInfo> _generateCdnExercises(List<Map<String, String>> data) {
    return data.map((ex) {
      ...
      final mp4List = [
        ...
        'standing side crunch', 'mountain climber', 'leg raise', 'v up', 'v-up'
      ];
      final String ext = mp4List.contains(normalizedName) ? 'mp4' : 'mov';

      // Special handling for V-Up naming mismatch
      if (normalizedName == 'v up' || normalizedName == 'v-up') {
        cdnBaseName = 'v_up'; // Force underscore if cdn uses underscore
      }
